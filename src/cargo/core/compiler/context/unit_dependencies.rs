...
type Plugins = HashMap<String, (PackageId, PluginCrateDeps)>;
...
pub fn build_unit_dependencies<'a, 'cfg>(
    ...
    for unit in roots.iter() {
        ...
        deps_of(unit, None, bcx, deps, profile_for, &plugins)?;
    }
...
fn deps_of<'a, 'cfg>(
    ...
    if !deps.contains_key(unit) {
        let unit_deps = compute_deps(unit, parent_unit, bcx, profile_for, plugins)?;
        let parent_unit = Some(unit);
        let to_insert: Vec<_> = unit_deps.iter().map(|&(unit, _)| unit).collect();
        deps.insert(*unit, to_insert);
        for (unit, profile_for) in unit_deps {
            deps_of(&unit, parent_unit, bcx, deps, profile_for, plugins)?;
        }
    }
    Ok(())
}
...
fn compute_deps<'a, 'cfg>(
    ...
    if unit.mode.is_run_custom_build() {
        return compute_deps_custom_build(unit, bcx, plugins);
    } else if unit.mode.is_doc() && !unit.mode.is_any_test() {
        // Note: This does not include Doctest.
        return compute_deps_doc(unit, parent_unit, bcx, plugins);
    }
    ...
    if let Some(active_plugins) = &unit.profile.active_plugins {
        for plugin_name in active_plugins.split(",") {
            match plugins.get(plugin_name) {
                Some((pkg_id_dep, info)) => {
                    match bcx.get_package(pkg_id_dep) {
                        Ok(pkg) => {
                            // TODO: kick this copy
                    ...
                    for v in &info.dep_crates {
                        let pkgs: Vec<_> = bcx.packages
                            .package_ids()
                            .filter_map(|pkg_id| {
                                if pkg_id.name().as_str() == v.package_name().as_str() {
                                    Some(pkg_id.clone())
                                } else {
                                    None
                                }
                            }).collect();
                        for pkg_id_dep in pkgs {
                            // TODO: this takes all of them, but not necessarily the one we want
                            match bcx.get_package(&pkg_id_dep) {
                                Ok(pkg) => {
                                    // TODO: kick this copy
                                    pkg.targets().iter().find(|t| t.is_lib()).map(|t| {
    ...
    ret.extend(maybe_lib(unit, parent_unit, bcx, profile_for, plugins));
...
fn compute_deps_doc<'a, 'cfg>(
    ...
    if unit.target.is_bin() {
        ret.extend(maybe_lib(unit, parent_unit, bcx, ProfileFor::Any, plugins));
    }
    Ok(ret)
}
...
fn maybe_lib<'a>(
    ...
    unit.pkg.targets().iter().find(|t| t.linkable()).map(|t| {
        let mode = check_or_build_mode(unit.mode, t);
        let unit = new_unit(bcx, unit.pkg, t, profile_for, unit.kind.for_target(t), mode, parent_unit,
            plugins);
        (unit, profile_for)
    })
}
